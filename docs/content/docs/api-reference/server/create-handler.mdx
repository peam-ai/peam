...
function createHandler(options: CreateChatOptions): (request: Request) => Promise<Response>;
```
...
| `maxMessageLength` | `number`           | No       | Max length for a single message  |
| `searchIndexStore` | `SearchIndexStore` | No       | Search index store for retrieval |
